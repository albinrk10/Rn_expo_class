...
export function LoginScreen() {
  ...
  const handleLogin = async () => {
    if (!email.includes("@") || password.length < 6) {
      Alert.alert("Validación", "Ingrese correo válido y contraseña mínima de 6 caracteres.");
      return;
    }
  ...
  return (
    ...
    >
      ...
      <View className="rounded-[28px] border border-slate-200 bg-white p-5">
        ...
        <Pressable onPress={handleLogin} disabled={loading} className="mb-3 rounded-2xl bg-indigo-600 py-4">
          <Text className="text-center text-base font-black text-white">
            {loading ? "Ingresando..." : "Iniciar sesión"}
          </Text>
        </Pressable>

        <Pressable onPress={() => setShowRegister(true)} className="rounded-2xl border border-indigo-200 py-4">
