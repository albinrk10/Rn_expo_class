...
export function HomePedidosScreen() {
  ...
  const handleCreatePedido = async () => {
    ...
    try {
      ...
      setImageBase64(null);
      await loadPedidos();
      Alert.alert("Pedido creado", "El pedido se guardó correctamente en Firestore.");
    } catch (error) {
    ...
    <View className="flex-1 bg-slate-50">
      <ScrollView className="px-5 pt-14" keyboardShouldPersistTaps="handled">
        <View className="mb-5 flex-row items-start justify-between gap-3">
          ...
          <Pressable onPress={logoutUser} className="rounded-2xl bg-rose-100 px-4 py-3">
        ...
        <View className="mb-5 rounded-[28px] border border-slate-200 bg-white p-4">
          ...
          <TextInput value={cliente} onChangeText={setCliente} editable={!loading} placeholder="Nombre del cliente" className="mb-3 rounded-2xl border border-slate-200 px-4 py-3 text-slate-900 disabled:bg-slate-100" />
          <TextInput value={producto} onChangeText={setProducto} editable={!loading} placeholder="Producto" className="mb-3 rounded-2xl border border-slate-200 px-4 py-3 text-slate-900 disabled:bg-slate-100" />

          <View className="flex-row gap-3">
            <TextInput value={cantidad} onChangeText={setCantidad} editable={!loading} keyboardType="numeric" placeholder="Cantidad" className="mb-3 flex-1 rounded-2xl border border-slate-200 px-4 py-3 text-slate-900 disabled:bg-slate-100" />
            <TextInput value={precio} onChangeText={setPrecio} editable={!loading} keyboardType="numeric" placeholder="Precio" className="mb-3 flex-1 rounded-2xl border border-slate-200 px-4 py-3 text-slate-900 disabled:bg-slate-100" />
          </View>
          ...
          <Pressable onPress={handlePickImage} disabled={loading} className="mb-3 rounded-2xl bg-slate-100 py-3 disabled:opacity-60">
            <Text className="text-center font-black text-slate-700">
              {imageUri ? "Cambiar evidencia Base64" : "Seleccionar evidencia Base64"}
            </Text>
          </Pressable>

          <Pressable onPress={handleCreatePedido} disabled={loading} className="rounded-2xl bg-indigo-600 py-4 disabled:bg-indigo-300">
            <Text className="text-center font-black text-white">Crear pedido</Text>
          </Pressable>
        </View>
